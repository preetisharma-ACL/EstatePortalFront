...
function SocialIcon(props: { label: string; href: string; brand: string; viewBox: string; children: JSX.Element }) {
  return <a href={props.href} target="_blank" rel="noopener noreferrer" aria-label={props.label} class={`grid h-9 w-9 place-items-center rounded-full text-white shadow-[0_6px_14px_-8px_rgba(14,27,51,0.85)] transition-all hover:-translate-y-0.5 hover:brightness-110 hover:shadow-[0_10px_18px_-8px_rgba(14,27,51,0.9)] ${props.brand}`}><svg height="15" viewBox={props.viewBox} fill="currentColor" class="max-w-[15px]" aria-hidden="true">{props.children}</svg></a>;
}
